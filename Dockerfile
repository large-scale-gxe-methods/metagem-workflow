...
# Install METAGEM from source (and store version so cache rebuilds when METAGEM source code updates)
# Note: currently breaks cache if any branch updates, but could make this branch-specific with /GH/path/refs/heads/[BRANCH]
ADD https://api.github.com/repos/large-scale-gxe-methods/METAGEM/git/refs/heads version.json
RUN apt-get update && apt-get -y install git make libzstd-dev && \
  ...
  ls -l && \
  make && \
  mv /METAGEM/src/METAGEM /METAGEM/METAGEM
